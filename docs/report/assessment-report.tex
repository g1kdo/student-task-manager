...
\url{https://github.com/g1kdo/student-task-manager}.
...
\midrule
1 & Add a real build file and make CI run the tests & Done --- \code{16748eb}, \code{668df32} \\
...
4 & Get printing out of \code{TaskManager} & Done --- \code{40f294b} \\
...
\bottomrule
\endfoot
1 & No build file anywhere, so \code{mvn test} could never work and the tests were unrunnable outside IntelliJ & Maven build targeting Java 21 declaring \code{junit-jupiter}, surefire failing the build on a failing test, plus a wrapper so it runs on a clean machine & \code{16748eb} & \code{test-run.txt} \\
\addlinespace
2 & The only test step was commented out; a pipeline that cannot fail is not a pipeline & Workflow compiles, runs the suite, packages the jar, produces coverage and smoke-tests the artefact; triggers cover \code{develop} too & \code{668df32} & \code{pipeline-fails-\allowbreak on-failing-test.txt} \\
\addlinespace
3 & ``Logging'' was two hardcoded printlns, no levels or timestamps, nothing for complete or delete & SLF4J + Logback; all four operations log with timestamps and levels; \code{WARN}/\code{ERROR} used meaningfully; console plus rolling file & \code{40f294b} & \code{demo-session.txt} \\
\addlinespace
4 & The health check printed a fixed string and inspected nothing & Reads the task store, measures heap against a 90\% threshold, reports uptime and counts; \code{UP}/\code{DEGRADED}/\code{DOWN}; never throws & \code{353202f} & \code{HealthCheckTest} \\
\addlinespace
5 & \code{nextInt()} then \code{nextLine()} made ``Input a valid digit'' print after every complete and delete & All input read by line through one prompt; rendering moved into a new \code{ConsoleApp} taking its streams as arguments & \code{40f294b} & two \code{REGRESSION} tests \\
\addlinespace
6 & Two tests, happy paths of add and complete only & 69 tests across four classes; out-of-range indexes parametrically including \code{MIN\_VALUE}/\code{MAX\_VALUE}; blank names, repeat completion, end of input & \code{3fa4f41}, \code{84fe78b}, \code{f0e1860} & \code{test-run.txt}, \code{coverage.txt} \\
\addlinespace
7 & No sprint review and no retrospective anywhere & Four documents written, with the timing stated honestly rather than backdated & \code{ca7300b}, \code{1b692e4} & \code{docs/sprint*.md} \\
\addlinespace
8 & Screenshots and a PDF committed inside \code{src/app/} & Restructured to the Maven standard layout; deliverables moved to \code{docs/}; the IntelliJ \code{.iml} dropped & \code{8e8925d} & repository tree \\
\addlinespace
9 & The one later commit added an unrelated \code{StreamArrayExample} & Moved to an \code{examples} package so \code{app} holds only assessment code, and excluded from the coverage gate & \code{8e8925d} & \code{src/main/java/\allowbreak examples/} \\
\addlinespace
...
fail. That criticism is demonstrably correct, and the run history proves it:
the old workflow reports \textbf{success} on commit \code{729b878}, whose
entire purpose was to comment out the test command. A pipeline that passes the
commit disabling its own testing is not verifying anything.

\subsubsection*{A real defect the local build could not catch}

The new pipeline's first two runs \emph{failed}, and they failed usefully:
...
\noindent The Maven wrapper was generated on Windows, where Git's
\code{core.filemode} is \code{false}, so Git stored \code{mvnw} as mode
\code{100644} rather than \code{100755}. Every local build had run green
throughout --- Git Bash on Windows ignores the permission bit --- so this was
only ever observable on the Linux runner. It was fixed at the root with
\code{git update-index --chmod=+x mvnw}, which stores mode \code{100755},
rather than by adding a \code{chmod +x mvnw} step to the workflow: that would
have hidden the cause instead of removing it, which is the same mistake as
commenting out a failing test step. The following runs are green on every
step, smoke test included.

This is the difference between the two pipelines in one sentence. The old one
passed a commit that switched testing off; the new one caught a packaging
defect that no local run could reveal. The full run table is in
\code{docs/evidence/ci-runs.txt}.

\subsubsection*{And it fails on a failing test}

Independently of the above, that a failing \emph{test} fails the build was also
demonstrated directly: one assertion in \code{TaskTest} was deliberately
inverted, \code{./mvnw clean verify} was run, and the result recorded before
the assertion was restored.
...
All runs are at
\url{https://github.com/g1kdo/student-task-manager/actions}; the table below is
taken from the public Actions API and reproduced in
\code{docs/evidence/ci-runs.txt}.
...
\toprule
\textbf{Run} & \textbf{Event} & \textbf{Commit} & \textbf{Result} & \textbf{Commit subject} \\
\midrule
CI \#4 & pull\_request & \code{3b3a5ff} & \textcolor{good}{\textbf{success}} & ci: mark mvnw executable so the Linux runner can run it \\
CI \#3 & push & \code{3b3a5ff} & \textcolor{good}{\textbf{success}} & ci: mark mvnw executable so the Linux runner can run it \\
CI \#2 & pull\_request & \code{5f076c9} & \textcolor{bad}{\textbf{failure}} & (the wrapper permission defect) \\
CI \#1 & push & \code{5f076c9} & \textcolor{bad}{\textbf{failure}} & (the wrapper permission defect) \\
\midrule
Java CI \#2 & push & \code{729b878} & \textcolor{bad}{\textbf{success}} & Comment out Maven test command in workflow \\
\bottomrule
\end{tabularx}
\caption{The new pipeline (top) went red on a real defect and green once it was
fixed. The old pipeline (bottom) reported success on the commit that switched
its own testing off --- the red \emph{success} is the point.}
\end{table}

For comparison, the first attempt's run --- checkout and setup-java only, with
no test step --- is archived at
...
\midrule
\code{8e8925d} & 2026-09-04 & chore: restructure repository into Maven standard layout \\
\code{16748eb} & 2026-09-04 & build: define a Maven build so the JUnit tests are runnable \\
\code{668df32} & 2026-09-04 & ci: run the tests in the pipeline instead of commenting them out \\
\code{40f294b} & 2026-09-04 & fix: read all console input by line, split rendering from the domain \\
\code{353202f} & 2026-09-04 & feat: make the health check inspect real state \\
\code{3fa4f41} & 2026-09-04 & test: cover the delete, invalid-index and blank-name paths \\
\code{84fe78b} & 2026-09-04 & test: cover the health check, including DEGRADED and DOWN \\
\code{f0e1860} & 2026-09-04 & test: drive the whole console menu, pinning the reported bug \\
\code{3c52c4f} & 2026-09-04 & docs: capture reproducible test, coverage and demo evidence \\
\code{ca7300b} & 2026-09-04 & docs: add the Sprint 1 review and retrospective \\
\code{1b692e4} & 2026-09-04 & docs: add the Sprint 2 review, retrospective and remediation record \\
\code{06a4146} & 2026-09-04 & docs: rewrite the README around the real build and deliverables \\
\code{ac114a7} & 2026-09-04 & docs: add the consolidated LaTeX report and compile it in CI \\
\code{b66a463} & 2026-09-04 & docs: harden the report source against compile failures \\
\multicolumn{3}{@{}l}{\small\itshape Later documentation commits follow; see \code{git log} for the current tip.} \\
\bottomrule
\end{tabularx}
...
The remediation work is a sequence of small commits, each one scoped change with a message
...
Codebase with incremental commit history
  & \url{https://github.com/g1kdo/student-task-manager}; Section~\ref{sec:delivery} \\
CI/CD evidence
...
Prototype demonstration
  & Section~\ref{sec:demo}; \code{docs/evidence/demo-session.txt} \\
