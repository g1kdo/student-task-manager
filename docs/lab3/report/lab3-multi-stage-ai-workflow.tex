...
\subsection{The approval gate, and what it could not catch}
\label{sec:gate}

The prompt's \emph{``show me the changes as a diff before applying anything''}
worked exactly as intended: Copilot presented the complete diff and asked
whether to apply or revise it, rather than writing to disk unilaterally. The
diff was reviewed, approved, and applied. Stage 2 took 8 minutes end to end.

That approved diff is captured verbatim in
\code{stage2/01b-copilot-proposed.diff}, and it contains this:
...
\noindent This is the defect Stage 3 later caught. It was present at the
approval gate, and it passed review.

That is the single most instructive result in this lab, and it is not a
criticism of the review. The assertion reads as entirely plausible: a test
named ``returned in due-date order'' asserting two names in due-date order.
Catching it by eye requires holding two due dates, two lead times and an
inclusive window in your head simultaneously, for one test among fourteen, in a
329-line diff. \textbf{A diff review checks intent; only execution checks
arithmetic.} The human gate and the machine gate are not substitutes for each
other, and the workflow needs both --- which is the strongest justification
available for why Stage 3 exists as a separate stage rather than a step the IDE
could have absorbed.
...
\subsection{And then the pipeline caught what Stage 3 missed}

Stage 3 finished green and the commit was pushed --- and \emph{CI failed}. The
feature had changed the console interaction contract: adding a task now prompts
for two optional fields, so each add consumes four lines of input instead of
two. The pipeline's smoke-test step drives the packaged jar over stdin with a
fixed keystroke sequence, and that sequence had gone stale. The application
read \code{2} --- intended as the ``view tasks'' menu choice --- as a due date:
...
\noindent The application behaved correctly: it rejected the bad date, said so,
and returned to the menu. The \emph{test} was wrong. But nothing in Stage 3 as
prompted could have found this, because \code{mvnw clean verify} does not run
the smoke test --- that step exists only in the pipeline, and it is the only
check that drives the built artefact the way a user would.

This is worth stating plainly rather than hiding, because it revises the
workflow's own design claim. Section~2 argues the CLI stage is where
\emph{execute, observe, react} belongs. That is right, but the Stage 3 prompt
scoped it to one command, and one command was not the whole verification. A
better prompt would say \emph{run every check the pipeline runs}, not
\emph{run the build}.

The smoke test was corrected --- and extended, since it was already being
touched: it now also asserts that a duplicate is rejected end-to-end and that
urgency renders in the list. The urgency assertion matches the set
\code{[OVERDUE|DUE\_TODAY|UPCOMING|NONE]} rather than a specific value, so it
does not rot as the calendar passes the test's due date.
...
\midrule
2 (IDE) & 8\,min end to end
  & Prompt submitted 17:10, Copilot finished 17:18, noted by the operator.
    Of that, $\approx$2\,min\,37\,s is file output (modification times
    17:14:10 to 17:16:47); the rest is the agent reading the five referenced
    files, reasoning, and waiting at the approval gate. \\
\addlinespace
...
\textbf{Human review and machine verification catch different classes of
error, and neither substitutes for the other.} This is the finding I did not
expect. The defective test was in the diff at the approval gate
(\S\ref{sec:gate}), it was reviewed, and it was approved --- because a test
named ``returned in due-date order'' asserting two names in due-date order
looks right. What was wrong was the arithmetic of a reminder window three lines
away. Then the same asymmetry repeated one level up: Stage 3's own
verification passed, and the \emph{pipeline} caught a stale smoke test that
\code{mvnw verify} does not run. Twice, a gate that looked sufficient was not,
and in both cases what closed the gap was executing something rather than
reading it. The practical rule I would carry forward: for every checking stage,
ask what class of error it structurally cannot see, and put a differently
shaped check after it.
